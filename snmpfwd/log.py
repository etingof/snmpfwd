...
class SyslogLogger(AbstractLogger):
    def init(self, *priv): 
        if not syslog:
            raise error.SnmpfwdError('syslog not supported on this platform')
        if len(priv) < 2:
            raise error.SnmpfwdError('Bad syslog params, need facility and priority')
        try:
            self.facility = getattr(syslog, 'LOG_%s' % priv[0].upper())
            self.priority = getattr(syslog, 'LOG_%s' % priv[1].upper())
        except AttributeError:
            raise error.SnmpfwdError('Unknown syslog option, need facility and priority names')
